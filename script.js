...
function addTaskToDom(tasks) {
  tasks.forEach(task => {
    ...
    newTask.innerHTML = 
      `<input type="checkbox" class="task-checkbox" data-id='${task._id}' data-description='${task.task.description}' ${task.task.done ? 'checked' : ''}>
       <span class="task-description ${task.task.done ? 'completed' : ''}">${task.task.description}</span>
       <i data-id='${task._id}' class="fas fa-trash-alt"></i>`;
    newTask.querySelector('.fa-trash-alt').addEventListener('click', removeTask);
    newTask.querySelector('.task-checkbox').addEventListener('click', taskCompleted);
...
document.getElementById("task-input-area").addEventListener("focus", function() {
